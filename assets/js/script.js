...
const nftCreateSection = document.getElementById('nftCreateSection');
...
const copyTradingSection = document.getElementById('copyTradingSection');
const signalsSection = document.getElementById('signalsSection');
const futuresSection = document.getElementById('futuresSection');
const helpCenterSection = document.getElementById('investmentSection');
...
const leverageBubble = document.getElementById("leverageBubble");
...
function updateThemeIcon() {
    ...
    if (themeToggleBtn) {
        ...
        if (tooltip) {
            tooltip.textContent = currentTheme === 'dark' ? 'Switch to Light Mode' : 'Switch to Dark Mode';
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const headerNavItems = document.querySelectorAll('.nav-main-item, .submenu-item');
    ...
    setTimeout(() => {
        ...
        tradingViewWidget = new TradingView.widget({
            ...
            style: '1',
            locale: 'en',
            toolbar_bg: currentTheme === 'dark' ? '#0a1120' : '#f1f5f9',
            ...
            overrides: {
                'paneProperties.background': currentTheme === 'dark' ? '#0a1120' : '#f8fafc',
                'paneProperties.vertGridProperties.color': currentTheme === 'dark' ? '#1e293b' : '#e2e8f0',
                'paneProperties.horzGridProperties.color': currentTheme === 'dark' ? '#1e293b' : '#e2e8f0',
                'symbolWatermarkProperties.transparency': 90,
                'scalesProperties.textColor': currentTheme === 'dark' ? '#94a3b8' : '#475569',
...
function navigateTo(page) {
    ...
    } else if (page === 'copy-trading') {
        copyTradingSection.style.display = 'block';
    ...
    } else if (page === 'nft-create') {
        nftCreateSection.style.display = 'block';
    } else if (page === 'nft-marketplace') {
        nftMarketSection.style.display = 'block';
    } else if (page === 'nft-collections') {
        nftCollectionSection.style.display = 'block';
    } else if (page === 'transaction-history') {
    ...
    document.querySelectorAll('.nav-item, .mobile-menu-item, .nav-main-item').forEach(item => {
...
function updateTradingViewTheme() {
    if (tradingViewWidget) {
        ...
        // Recreate the widget with the new theme
        tradingViewWidget = new TradingView.widget({
            symbol: symbol,
            interval: currentTimeframe,
            container_id: 'tv-chart',
...
function updateLeverageBubble() {
    const val = leverageRange.value;
    const min = leverageRange.min ? leverageRange.min : 1;
    const max = leverageRange.max ? leverageRange.max : 100;

    const percent = ((val - min) * 100) / (max - min);

    leverageBubble.innerText = `${val}x`;
    leverageBubble.style.left = `calc(${percent}% + (${8 - percent * 0.15}px))`; 
}

leverageRange.addEventListener("input", updateLeverageBubble);

// Initialize on load
updateLeverageBubble();
